...
async function createFile(callData, client) {
  ...
  if (callData.length > 4096) {
    await (await new _FileAppendTransaction.default().setFileId(fileId).setContents(hexedCallData.substring(4096, hexedCallData.length)).setChunkSize(4096).execute(client)).getReceipt(client);
  }
